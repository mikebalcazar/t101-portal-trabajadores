...
$LOG = Join-Path (Get-Location) "ultimo-despliegue.log"
"=== Despliegue $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ===" | Set-Content $LOG -Encoding utf8

function Apunta($t){ $t | Add-Content $LOG -Encoding utf8 }
...
function PonSecreto($nombre, $valor) {
  ...
  $valor | npx wrangler secret put $nombre 2>&1 | Out-String | Add-Content $LOG -Encoding utf8
